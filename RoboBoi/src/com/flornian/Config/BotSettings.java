...
public class BotSettings {
    ...
    public static boolean getValueBoolean(String variableName) throws IOException {
        //Get all the settings
        List<String> settings = IO.readSmallTextFile(Vars.settingsFile);
        ...
        for (String setting : settings) {
            if (setting.contains("=")) {
                if (setting.split("=")[0].equalsIgnoreCase(variableName)) {
                    return Boolean.parseBoolean(setting.split("=")[1]);
                }
            }
        }

        //No setting found, return false
        return false;
    }
